  ...
  Widget build(BuildContext context) {
    return Center(
      child: Container(
        ...
        width: 200,
        child: ListView(
          scrollDirection: Axis.vertical, //--список будет вертикальным
          children: const [
            MyListItem(number: 1,),
            MyListItem(number: 2,),
            MyListItem(number: 3,),
...
//с методом конструктора
class MySimpleListBuilder extends StatelessWidget {
  MySimpleListBuilder({Key? key}) : super(key: key);

  var list = List<int>.generate(50, (i) => i + 1);  //--список созданный генератором (50шт с правилом i+1)

  @override
  Widget build(BuildContext context) {
    return ListView.builder(
      padding: const EdgeInsets.all(8),
      itemCount: list.length, //--кол-во предметов в списке. Без него будет бесконечный список
